    ...
    <div className="w-full max-w-[440px]">
      ...
      <Card className="w-full border-[var(--border-default)] shadow-[var(--shadow-lg)]">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Enter your email and we&apos;ll send a 6-digit code you can use on the next step.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            className="space-y-4"
            onSubmit={handleSubmit((data) => forgot.mutate(data.email))}
          >
            ...
            <Button type="submit" className="min-h-11 w-full" disabled={forgot.isPending}>
              {forgot.isPending ? "Sending…" : "Send reset code"}
            </Button>
          </form>
          <p className="mt-4 text-center text-sm text-[var(--text-muted)]">
            Have a code already?{" "}
            <Link href="/reset-password" className="font-semibold text-[var(--brand-primary)]">
              Enter code
            </Link>
          </p>
